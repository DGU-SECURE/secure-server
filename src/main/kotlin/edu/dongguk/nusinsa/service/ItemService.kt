...
class ItemService(
    ...
    fun findItems(
        ...
        val itemList: Page<Item> = when {
            ...
            category == null -> {
                itemRepository.findAllByName(paging,name!!)
            }
            else -> {
                var itemCategory : ItemCategory = ItemCategory.valueOf(category);
                itemRepository.findAllByCategory(paging,itemCategory)
            }
        }
